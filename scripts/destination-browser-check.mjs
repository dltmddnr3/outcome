...
try {
  ...
  for (const width of [390, 1440]) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      await page.getByRole('button', { name: i === 7 ? '목적지 검토' : '다음 질문', exact: true }).click()
      if (i === 0) {
    ...
    const reviewDetails=page.locator('.destination-studio__review-details')
    assert.equal(await reviewDetails.getAttribute('open'),null)
    assert.equal(await page.locator('dl>div:visible').count(),3)
    const axSession=await page.context().newCDPSession(page)
    const reviewNames=async()=> (await axSession.send('Accessibility.getFullAXTree')).nodes.filter(node=>!node.ignored).map(node=>node.name?.value)
    assert.equal((await reviewNames()).includes('문제 수정'),false)
    await reviewDetails.locator('summary').focus();await page.keyboard.press('Enter')
    assert.equal((await reviewNames()).includes('문제 수정'),true)
    assert.equal(await page.locator('dl>div:visible').count(),8)
    ...
    await page.getByRole('button', { name: '목적지 검토', exact: true }).click()
    await page.locator('.destination-studio__review-details summary').click()
    ...
    for(let index=0;index<7;index++) {
      ...
      await storagePage.getByRole('button',{name:index===6?'목적지 검토':'다음 질문',exact:true}).click()
    }
    await storagePage.locator('.destination-studio__review-details summary').click()
    ...
    await analysisPage.locator('.destination-studio__analysis-tools > summary').click()
