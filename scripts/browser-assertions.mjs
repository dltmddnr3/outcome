...
export async function measureDashboard(page) {
  return page.evaluate(({ requiredGithubLabels }) => {
    ...
    const sourceGroupSection = detail?.querySelector('[data-source-groups]'); const strayGroupSections = [...(detail?.querySelectorAll('.oc-detail-grid > section') ?? [])].filter((section) => !section.hasAttribute('data-selected-gates') && !section.hasAttribute('data-source-groups')); const meaningfulGroupVisibility = strayGroupSections.length === 0 && (!sourceGroupSection || gateGroups.length > 0 && gateGroups.every((group) => group.dataset.generic === 'false' && group.querySelector('strong')?.textContent?.trim() && group.querySelector('small')?.textContent?.startsWith('코드 ')))
    ...
    const nowSummary = root.querySelector('.oc-now-summary'); const staleNowCount = (nowSummary?.textContent?.match(/관측 오래됨/g) ?? []).length; const singleStaleNowSignal = nowSummary?.dataset.nowStatus !== 'stale' || staleNowCount === 1
...
export function assertDashboardMeasurement(name, result) {
  ...
  for (const key of ['pageHeading', 'sequentialHeadings', 'heroGateContract', 'noDuplicateProgress', 'compactRoles', 'liveSemantics', 'roleCopyPolish', 'heroPrimaryIdentity', 'singleStaleNowSignal', 'activityBand', 'unifiedFlow', 'funnelCounts', 'funnelPurpose', 'placementOnly', 'scopeJourney', 'explorerSemantics', 'listboxOwnership', 'explorerGeometry', 'allStagesDiscoverable', 'explorationHonest', 'detailSemantics', 'detailGateTruth', 'currentDetailDedup', 'adaptiveGateGroups', 'meaningfulGroupVisibility', 'timingHonesty', 'noFabricatedProgress', 'contentPreservation', 'technicalCollapsed', 'technicalEvidence', 'mobileAuthoritativeOrder']) if (!result[key]) failures.push(`${key}=false${key === 'scopeJourney' ? `:${JSON.stringify(result.scopeDebug)}` : ''}`)
...
export async function verifyAllDashboardStates(page, viewportName) {
  ...
  let measuredStates = 0; let minimumFocusContrast = Infinity; let maximumActiveAnimations = 0; let maximumScopeConnectorGap = 0; let sourceGroupStates = 0; let genericGroupSections = 0; let maximumStaleNowCount = 0; let layoutReceipt = null; const failures = []; const counts = []
  for (let projectIndex = 0; projectIndex < 2; projectIndex += 1) {
    ...
    for (let stageIndex = 0; stageIndex < stageCount; stageIndex += 1) {
      ...
      maximumActiveAnimations = Math.max(maximumActiveAnimations, result.activeAnimationCount); maximumScopeConnectorGap = Math.max(maximumScopeConnectorGap, result.scopeConnectorGap); sourceGroupStates += result.sourceGroupSectionCount; genericGroupSections += result.genericGroupSectionCount; maximumStaleNowCount = Math.max(maximumStaleNowCount, result.staleNowCount)
  ...
  if (sourceGroupStates !== 1) failures.push(`${viewportName}: expected one source-labeled group state, got ${sourceGroupStates}`)
  if (genericGroupSections !== 0) failures.push(`${viewportName}: generic group sections=${genericGroupSections}`)
  if (failures.length) throw new Error(`${viewportName}: measured projects=2 selectedStages=${measuredStates}; ${failures.join(' || ')}`)
  console.log(`${viewportName}: projects=2 selectedStages=${measuredStates} sourceOptions=${counts.join(',')} sourceGroupStates=${sourceGroupStates} genericGroupSections=${genericGroupSections} staleNowCount<=${maximumStaleNowCount} heroGauge=1-or-0 roles=4 roleHeights=${layoutReceipt.roleHeights.map((value) => value.toFixed(0)).join('/')} activityRatio=${layoutReceipt.activityRatio.toFixed(3)} flowDelta=${layoutReceipt.flowLeftDelta.toFixed(2)}/${layoutReceipt.flowWidthDelta.toFixed(2)}px explorer=${layoutReceipt.explorerMasterWidth.toFixed(0)}px/${layoutReceipt.explorerGap.toFixed(0)}px scope=${layoutReceipt.scopeOrientation} scopeConnectorMaxGap=${maximumScopeConnectorGap.toFixed(2)}px activeAnimation<=${maximumActiveAnimations} reducedMotionStatic=true unexpectedEnglish=0 translationFallback=0 clipped=0 ellipsis=0 intersections=0 viewportEscape=0 documentOverflow=0 controls>=44 text>=11 textContrast>=4.5 focusContrast>=${minimumFocusContrast.toFixed(2)}`)
}
