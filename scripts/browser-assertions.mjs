...
export async function measureDashboard(page) {
  return page.evaluate(({ requiredGithubLabels }) => {
    ...
    const explorerSemantics = root.querySelectorAll('.oc-stage-list[role="listbox"]').length === 1 && options.length > 0 && selectedOptions.length === 1 && currentOptions.length === 1 && tabStops.length === 1 && root.querySelectorAll('.oc-stage-list [aria-pressed]').length === 0
    ...
    const exploring = selectedStageId !== currentStageId; const currentName = currentOptions[0]?.querySelector('strong')?.textContent?.trim(); const explorationHonest = detail?.dataset.exploring === String(exploring) && (exploring ? detail.querySelector('.oc-exploration-badge')?.textContent.includes(currentName) : !detail.querySelector('.oc-exploration-badge') && detail.querySelector('.oc-selection-status')?.textContent.includes('현재 작업 단계 상세'))
    const detailText = detail?.innerText ?? ''; const detailState = detail?.dataset.stageState; const detailSemantics = Boolean(detailState) && !detail.querySelector('.oc-confirmed') && (detailState === 'complete' ? detailText.includes('증거 확정 / 전체') : detailText.includes('체크됨 / 전체'))
...
export function assertDashboardMeasurement(name, result) {
  ...
  for (const key of ['pageHeading', 'sequentialHeadings', 'heroGateContract', 'noDuplicateProgress', 'compactRoles', 'liveSemantics', 'activityBand', 'unifiedFlow', 'funnelCounts', 'funnelPurpose', 'placementOnly', 'scopeJourney', 'explorerSemantics', 'explorerGeometry', 'allStagesDiscoverable', 'explorationHonest', 'detailSemantics', 'timingHonesty', 'noFabricatedProgress', 'contentPreservation', 'technicalCollapsed', 'technicalEvidence', 'mobileAuthoritativeOrder']) if (!result[key]) failures.push(`${key}=false${key === 'scopeJourney' ? `:${JSON.stringify(result.scopeDebug)}` : ''}`)
