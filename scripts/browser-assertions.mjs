...
export async function measureDashboard(page) {
  const result = await page.evaluate(({ requiredGithubLabels }) => {
    ...
    const gateGauge = heroGate?.querySelector('.oc-gate-gauge'); const gateClosed = Number(gate?.dataset.closed); const gateTotal = Number(gate?.dataset.total); const expectedGatePercent = gateTotal > 0 ? Math.round(gateClosed / gateTotal * 100) : null
    const currentGateTruth = heroGate?.dataset.gateFill === 'available' ? Boolean(gateGauge && heroGate.textContent.includes('현재 작업 단계 완료 조건') && heroGate.textContent.includes(`${gateClosed}/${gateTotal} · ${expectedGatePercent}%`) && gateGauge.getAttribute('aria-label')?.includes(`${expectedGatePercent}%`) && document.querySelectorAll('.oc-gate-gauge').length === 1) : !gateGauge && !/%/.test(heroGate?.querySelector('strong')?.textContent ?? '')
    const placementOnly = [phase, scope, stage].every((row) => { const value = row?.querySelector('.oc-funnel-heading small')?.textContent ?? ''; return /\d+\s*\/\s*\d+/.test(value) && !value.includes('%') })
    const timing = document.querySelector('.oc-now-timing'); const elapsed = timing?.querySelector('.oc-time-elapsed'); const eta = timing?.querySelector('.oc-time-eta')
    ...
    const contentPreservation = ['.oc-hero', '.oc-now-summary', '.oc-bindings', '.oc-current-flow', '.oc-funnel-gate', '.oc-current-stage', '.oc-stage-explorer', '.oc-selected-detail', '.oc-technical'].every((selector) => Boolean(document.querySelector(selector))) && document.querySelectorAll('.oc-bindings article').length === 4
    const columnCount = (element) => getComputedStyle(element).gridTemplateColumns.split(' ').filter(Boolean).length
    const scopeRail = document.querySelector('.oc-scope-rail'); const scopeItems = [...scopeRail.querySelectorAll('article')]; const activeScope = scopeItems.filter((item) => item.dataset.railState === 'active')
    const scopeJourney = innerWidth <= 600
      ? columnCount(scopeRail) === 2 && scopeItems.every((item) => Boolean(item.querySelector('svg') && item.querySelector('small')?.textContent?.trim()))
      : getComputedStyle(scopeRail).display === 'flex' && getComputedStyle(scopeRail).flexWrap === 'nowrap' && scopeItems.every((item, index) => index === scopeItems.length - 1 || getComputedStyle(item, '::after').display !== 'none') && activeScope.length <= 1 && activeScope.every((item) => getComputedStyle(item.querySelector(':scope > span')).transform !== 'none' && getComputedStyle(item.querySelector(':scope > span')).borderColor === 'rgb(173, 255, 47)')
    const alpha = (value) => { const channels = value.match(/[\d.]+/g)?.map(Number) ?? []; return channels.length > 3 ? channels[3] : value === 'transparent' ? 0 : 1 }
    const connectorChecks = activeScope.map((item) => { const index = scopeItems.indexOf(item); const previous = scopeItems[index - 1]; const node = item.querySelector(':scope > i'); const previousRect = previous?.getBoundingClientRect(); const nodeRect = node?.getBoundingClientRect(); const connectorStyle = previous ? getComputedStyle(previous, '::after') : null; const connectorRight = previousRect && connectorStyle ? previousRect.left + Number.parseFloat(connectorStyle.left) + Number.parseFloat(connectorStyle.width) : null; const gap = previous?.dataset.railState === 'complete' && nodeRect && Number.isFinite(connectorRight) ? Math.abs(nodeRect.left - connectorRight) : 0; return { background: getComputedStyle(item).backgroundColor, border: getComputedStyle(item).borderColor, gap } })
    const scopeConnectorContinuity = innerWidth <= 600 || connectorChecks.every((check) => alpha(check.background) === 0 && alpha(check.border) === 0 && check.gap <= 1)
    const scopeConnectorGap = connectorChecks.reduce((maximum, check) => Math.max(maximum, check.gap), 0)
    const activeScopeWrapperBackgrounds = connectorChecks.map((check) => check.background)
    ...
    const repeatingSemanticElements = liveCards.filter((card) => getComputedStyle(card, '::before').animationName !== 'none').length + liveCards.filter((card) => [...card.querySelectorAll('.oc-live-bars i')].some((bar) => getComputedStyle(bar).animationName !== 'none')).length
...
export function assertDashboardMeasurement(name, result) {
  ...
  for (const key of ['currentNextReadable', 'allStagesDiscoverable', 'selectedStageExposed', 'sourceStatusText', 'hero', 'pageHeading', 'sequentialHeadings', 'mobileTwoColumns', 'nowStaleHonesty', 'standaloneHeadingAbsent', 'funnelCounts', 'funnelPurpose', 'funnelShape', 'railSemantics', 'heroFillSemantics', 'currentGateTruth', 'placementOnly', 'timingHonesty', 'contentPreservation', 'scopeJourney', 'liveSemantics', 'explorationHonest', 'technicalCollapsed', 'technicalEvidence', 'mobileAuthoritativeOrder', 'bottomShellState', 'detailSemantics']) if (!result[key]) failures.push(`${key}=false`)
  if (!result.scopeConnectorContinuity) failures.push(`scopeConnectorContinuity=false backgrounds=${result.activeScopeWrapperBackgrounds.join(',')} gap=${result.scopeConnectorGap.toFixed(2)}`)
  if (result.activeAnimationCount > 1) failures.push(`activeAnimationCount=${result.activeAnimationCount}`)
  if (result.repeatingSemanticElements > 2) failures.push(`repeatingSemanticElements=${result.repeatingSemanticElements}`)
...
async function assertReducedMotion(page, name) {
  ...
  const repeating = await page.evaluate(() => [...document.querySelectorAll('.oc-dashboard *')].filter((element) => getComputedStyle(element).animationIterationCount === 'infinite' && getComputedStyle(element).animationName !== 'none').length + [...document.querySelectorAll('.oc-dashboard *')].filter((element) => getComputedStyle(element, '::before').animationIterationCount === 'infinite' && getComputedStyle(element, '::before').animationName !== 'none').length)
...
export async function verifyAllDashboardStates(page, viewportName) {
  ...
  let measuredStates = 0; let minimumFocusContrast = Infinity; let maximumActiveAnimations = 0; let maximumScopeConnectorGap = 0; const failures = []; const axisClipStages = new Set(); let axisClipCount = 0; const gateRowTops = []
  for (let projectIndex = 0; projectIndex < 2; projectIndex += 1) {
    ...
    for (let stageIndex = 0; stageIndex < stageCount; stageIndex += 1) {
      ...
      maximumScopeConnectorGap = Math.max(maximumScopeConnectorGap, result.scopeConnectorGap)
  ...
  console.log(`${viewportName}: projects=2 selectedStages=${measuredStates} gateRowTops=${gateRowTops.join(',')} gateProgress=true timingHonest=true scopeJourney=true scopeWrapper=${/(?:desktop|landscape)/.test(viewportName) ? 'transparent' : 'mobile-preserved'} scopeConnectorMaxGap=${maximumScopeConnectorGap.toFixed(2)}px funnelCounts=true technicalCollapsed=true activeAnimation<=${maximumActiveAnimations} repeatingSemantic<=2 reducedMotionStatic=true unexpectedEnglish=0 translationFallback=0 clipped=0 ellipsis=0 intersections=0 viewportEscape=0 documentOverflow=0 controls>=44 text>=11 textContrast>=4.5 mobileOrder=true focusContrast>=${minimumFocusContrast.toFixed(2)}`)
}
