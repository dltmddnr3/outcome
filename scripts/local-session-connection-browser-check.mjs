...
const receiver=await createLocalSessionReceiver({previewOrigin:origin,verifySession:async token=>token==='fixture-session',timeoutMs:10000})
let browser
try{
  ...
  await page.getByRole('button',{name:'이 기기의 실행기에 연결'}).click()
  if(!granted){
  ...
  try{await page.getByRole('status').filter({hasText:'로그인 연결을 확인했습니다.'}).waitFor({timeout:5000})}
