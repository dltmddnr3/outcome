...
try{
 ...
 for(const width of [390,1440])for(const mode of ['success','unavailable','ambiguous-recorded','ambiguous-missing']){
  ...
  let posts=0,reads=0,prepares=0,receipt=null
  ...
  if(mode==='unavailable'){
   await panel.getByText('현재 초안의 근거 검증을 완료하지 못했습니다.',{exact:false}).waitFor()
   ...
   if(mode!=='ambiguous-missing'){
    await panel.getByText('확정 요청 기록됨 · 프로젝트 생성 대기',{exact:true}).waitFor()
    assert.equal(await prepare.isDisabled(),true);assert.equal(await confirm.count(),0)
    await page.reload();await open();await read.click()
    await panel.getByText('확정 요청 기록됨 · 프로젝트 생성 대기',{exact:true}).waitFor()
    const cdp=await page.context().newCDPSession(page),ax=await cdp.send('Accessibility.getFullAXTree')
    for(const text of ['확정 요청 기록됨 · 프로젝트 생성 대기','completionAuthority=false'])assert.ok(ax.nodes.some(n=>n.name?.value===text))
    await cdp.detach()
   }
  ...
  console.log(JSON.stringify({width,mode,posts,reads,prepares,actualStudio:true,noAutomaticConfirmation:true,overflow:false,pageErrors:0}))
  await page.close()
 }
