...
try {
  ...
  const check = async (base, label, localAssets = false) => {
    ...
    const page = await browser.newPage(); await page.goto(`${base}/cherry-note-dashboard`); await page.getByText('큰 단계에서 완료 조건까지', { exact: true }).waitFor(); const renderedUI = await page.locator('body').innerText(); await page.close()
