...
- [ ] A2: Audit verifies standalone startup, local-only privacy, source isolation, build reproducibility, and rollback.
  PROVES: release_audit
  EVIDENCE: b57edd7 audit `FAIL` remains authoritative. Candidate 9580c45 corrected UUID and PID boundaries, but fresh affected QA `docs/STAGE7_CORRECTION_FRESH_UX_QA_9580c45.md` found nine `/tmp/...` absolute paths in public Gate evidence and a matching scanner blind spot. The R1B-1 Builder correction removes only raw Gate evidence from public Package projection, preserves Stage evidence axes, expands path sanitation/scanning, and passes local gates. The no-restart live origin still serves the prior module and fails `public:api:localPath`; A2 remains open pending exact-candidate activation and fresh audit.
