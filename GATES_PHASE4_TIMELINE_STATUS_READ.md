...
## Git-directory fixture correction — Builder verification (2026-09-06)

New bounded authority SHA-256 `9c8432aa438ace2dc463146776f9730f6d4d8b42f8db42b34dc59e129d7fdc5e` preserves the same `AP-4-READ-STATUS / T1-T8` target and exact base `9a61506f9838041eb5f77dce4999ddf15b40bf83`. Independent diagnosis SHA-256 `e0c8ff7984e56908145b8f4602497cb54884b82b32c20e2f343cafea08d18e84` established that S2 failed because the test called regular-file `unlinkSync` on a copied `.git` directory, not because of filesystem permission denial.

The sole test change replaces that one `.git` removal with existing `rmSync(..., { recursive: true, force: true })`. The regular snapshot-file `unlinkSync` and every negative predicate/assertion remain unchanged. Exact diff is one insertion/one deletion in `server/outcome-current-projection.test.mjs`; product sources are unchanged.

CHECK: run `node --test server/outcome-current-projection.test.mjs` once in an isolated full clone whose `.git` is a directory and once in its isolated linked worktree whose `.git` is a regular file.
EXPECT: both native exits `0`; `11/11` pass in each; the no-resolvable-HEAD case still returns `cold_compile_required / canonical_source_unavailable`; the missing-snapshot regular-file unlink regression also remains green.
