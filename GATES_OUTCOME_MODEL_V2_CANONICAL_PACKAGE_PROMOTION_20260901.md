...
Status: **D1 CLOSED · BUILDER CANDIDATE DISPATCH READY · ACTIVE ROOT CUTOVER LOCKED**
...
- [ ] B1: Builder anchors the exact accepted source on one durable isolated candidate branch and adds only the bounded implementation, projection, tests and receipt needed by this Gate.
...
- [ ] B2: Builder replaces the historical one-shot canary pattern with a deterministic, source-addressed Model v2 Current Projection compiler and immutable local projection artifact.
...
- [ ] B3: Builder proves local rollback and compatibility while preserving the accepted local-only authority boundary.
...
- [ ] Q1: A fresh independent UX & Product QA task validates the exact B1-B3 candidate read-only.
...
- [ ] A1: A separate fresh Release Audit validates exact lineage, scope, rollback, privacy, test evidence and active-root non-mutation.
...
- [ ] C1: Cherry accepts the independently verified canonical-package candidate before any active-root cutover.
