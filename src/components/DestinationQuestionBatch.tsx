...
export function DestinationQuestionBatch({context,receipt,onSave,stageOnly=false}:{context:DiscoveryContext;receipt:string;onSave:(next:DiscoveryContext)=>Promise<void>;stageOnly?:boolean}) {
 ...
 const save=async()=>{
  ...
  try {
   ...
   setSaved(true);setNotice(stageOnly?'답변을 이 화면에 보관했습니다. 서버 저장 전이며 버전 갱신 확인이 필요합니다.':'후속 답변이 저장되었습니다. 다음 질문은 새 답변을 기준으로 받아야 합니다. 목적지 확정은 아닙니다.')
  }catch{
   if(latest.current!==captured)return
 ...
 return <section className="destination-studio__question" aria-label="목적지 후속 질문" aria-busy={busy}>
  ...
  <button className="destination-studio__primary" type="button" disabled={!ready||validatedIdentity!==identity||!questions.length||busy||hold||saved||questions.some(q=>!values[q.id]?.trim())} onClick={()=>void save()}>{stageOnly?'이전 질문 답변 검토 · 아직 미저장':'후속 답변 저장'}</button>
 </section>
}
