...
export type CherryNoteDashboardData = {
  ...
  live: { state: 'active' | 'complete' | 'stale' | 'unknown'; role: string; activity: string; observedAt: string | null; elapsedSeconds: number | null; processLabel: string; pulse: { passed: number | null; failed: number; delta: number | null; currentTest: string | null } }
  progress: { total: number; implemented: number; tested: number; evidenceClosed: number; percentages: { implemented: number; tested: number; evidenceClosed: number } }
...
export const gateGroupDisplayNames: Record<string, string> = { Y: '링크 미리보기', L: '아이보리 표면 통일', B: '브랜드 표현', M: '더보기 화면', N: '새 폴더 만들기', E: '새 일정 만들기', A: '폴더 보관·복원', D: '폴더·콘텐츠 복구 삭제', G: '엔지니어링 완료 증거' }
const gateDescriptions: Record<string, string> = { Y: '링크 제목·썸네일·복사 동작', L: '모든 화면의 아이보리 표면 통일', B: '로고·브랜드 색상과 표현 일치', M: '더보기 화면의 간격과 정보 구조', N: '폴더 생성·상위 폴더 선택 흐름', E: '일정 입력·저장과 키보드 흐름', A: '폴더 보관·복원 동작', D: '폴더와 콘텐츠의 복구 가능한 삭제', G: '전체 회귀·증거·배포 후보 확정' }
...
function ProgressLayer({ label, value, total, percent, tone }: { label: string; value: number; total: number; percent: number; tone: string }) { return <article className={`cn-progress-layer ${tone}`}><div><span>{label}</span><strong>{value}<small>/{total}</small></strong></div><i aria-label={`${label} ${percent}%`}><em style={{ width: `${percent}%` }} /></i><b>{percent}%</b></article> }
...
export function CherryNoteDashboard({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  return <section className="cn-dashboard">
    ...
    <div className="cn-dashboard-hero"><div><span>OUTCOME</span><strong>{dashboard.project.outcome}</strong><small>기능 + UI/UX + 실제 사용 + 독립 QA/감사 + Cherry 수용</small></div><div><small>NOW · {dashboard.live.role}</small><strong>{dashboard.live.activity}</strong><p>{dashboard.live.processLabel} · {formatDuration(dashboard.live.elapsedSeconds)}</p></div></div>
