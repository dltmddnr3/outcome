...
describe('OUTCOME Package dashboard', () => {
  ...
  it('nine zero-denominator timeline events expose truthful rendered and accessible text at all target viewports', () => {
    ...
    for (const viewport of ['1440x900', '1024x768', '390x844', '320x568']) {
      ...
      const timelineText = [...markup.matchAll(/<section class="oc-result-timeline"[\s\S]*?<\/section>/g)].map((match) => match[0]).join('\n')
      expect(timelineText.match(/근거 닫힘 0\/0/g) ?? [], viewport).toHaveLength(0)
      expect(timelineText.match(/완료 조건 미연결 · 전체 완료율 아님/g) ?? [], viewport).toHaveLength(9)
      expect(timelineText, viewport).toContain('근거 닫힘 1/2')
    }
  })
  ...
  it('왼쪽 레일은 프로젝트 작업공간과 정직한 준비 중 기능만 제공한다', () => {
    ...
    const source = OutcomeDashboard.toString() + WorkspaceManagement.toString()
