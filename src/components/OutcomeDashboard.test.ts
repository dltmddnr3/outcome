...
describe('OUTCOME Package dashboard', () => {
  ...
  it('keeps approval observation accent bounded and decision targets accessible without motion', () => {
    ...
    const rows = ['문제', '대상 사용자', '결과', '범위', '비목표', '제약', '완료 판정', '정상 실패', '복구'].map((item, index) => ({ key: `row-${index}`, item, value: `${item} 내용`, source: index % 2 === 0 ? '답변' as const : '미결' as const }))
    const markup = renderToStaticMarkup(createElement(ApprovalInbox, { projection: modelV2(), reviewItem: { rows } }))
    expect(markup).toContain('data-approval-kind="review"')
    expect(markup).toContain('class="oc-destination-review"')
    expect(markup).toContain('aria-labelledby="destination-review-title"')
    expect((markup.match(/data-review-item=/g) ?? [])).toHaveLength(9)
    expect((markup.match(/aria-label="고치기 · /g) ?? [])).toHaveLength(9)
    expect(new Set([...markup.matchAll(/aria-label="고치기 · ([^"]+)/g)].map((match) => match[1])).size).toBe(9)
    expect(markup).not.toMatch(/<table|<th|role="table"|필수|required/)
    expect(markup).toContain('확정 조건이 아직 정해지지 않았어요 · 검토는 읽기 전용')
    expect(effectiveProperty(styles, '.oc-destination-review dt', 'font-size')).toBe('11px')
    expect(effectiveProperty(styles, '.oc-destination-review dd.oc-destination-review__source', 'font-size')).toBe('11px')
    expect(effectiveProperty(styles, '.oc-destination-review dd.oc-destination-review__actions', 'margin-top')).toBe('4px')
    const destinationRules = styles.slice(styles.indexOf('.planner-conversation__actions button:disabled{cursor:not-allowed;opacity:.5}') + '.planner-conversation__actions button:disabled{cursor:not-allowed;opacity:.5}'.length).trim()
    expect(destinationRules.match(/[^{}]+\{[^{}]*\}/g)).toHaveLength(5)
    expect(destinationRules).not.toMatch(/background(?:-color)?:|#[0-9a-f]{3,8}|--oc-accent|transition|animation/i)
  })
