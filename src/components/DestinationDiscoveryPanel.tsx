...
import {captureDestinationReviewBinding,requestDestinationDiscovery,requestDestinationQuestions,requestDestinationQuestionRun,type DiscoveryQuestionRun,type StoredDestinationDraft,type StoredDiscovery} from '../lib/api'
...
export function DestinationDiscoveryPanel({intake}:{intake:StoredDestinationDraft}) {
 ...
 const act=async(kind:'load'|'initialize'|'questions'|'request')=>{
  ...
  try{
   if(kind==='questions'||kind==='request'){
    ...
    setDiscovery(value);setReceipt(null);setChecked(true);setHold(false);setRun(null);setRunChecked(false);setRunHold(false)
 ...
 const save=async(next:DiscoveryContext)=>{
  ...
  try{
   ...
   setDiscovery(stored);setReceipt(null);setRun(null);setRunChecked(false);setRunHold(false);setNotice('후속 답변이 저장되었습니다. 다음 질문은 새 답변 맥락을 기준으로 확인해 주세요. 목적지 확정은 아닙니다.')
  }finally{lock.current=false;setBusy(false)}
 }
