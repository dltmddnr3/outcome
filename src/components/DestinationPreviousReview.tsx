...
export function DestinationPreviousReview({intake,previous,decisions,pending,pendingReceipt,isCurrent,onCancel,onUpdated}:{intake:StoredDestinationDraft;previous:StoredDiscovery;decisions:DiscoveryDecisionReview;pending:boolean;pendingReceipt:string|null;isCurrent:()=>boolean;onCancel:()=>void;onUpdated:(value:StoredDiscovery)=>void}){
 const [ack,setAck]=useState(false),[busy,setBusy]=useState(false),[attempted,setAttempted]=useState(false)
 const [staged,setStaged]=useState<DiscoveryContext|null>(null)
 const unresolved=pending&&!staged
 ...
 const update=async()=>{
  if(lock.current||attempted||unresolved||!ack)return
  ...
  try{
   ...
   const context={...(staged??previous.context),revision:previous.revision+1,mode:document.mode,source:document.source,seedAnswers:document.answers,unknowns:document.unknowns}
 ...
 return <section aria-label="이전 버전 후속 답변 비교" aria-busy={busy}>
  ...
  {pendingReceipt&&<><p>이전 맥락에서 발행된 질문입니다. 현재 초안과의 차이를 고려해 직접 답해 주세요. 답변 검토만으로 저장하지 않습니다.</p><DestinationQuestionBatch context={previous.context} receipt={pendingReceipt} stageOnly onSave={async next=>{if(!isCurrent()||attempted||lock.current)throw Error('review_unavailable');setStaged(next);setAck(false)}}/></>}
  {unresolved?<p role="status">이전 맥락에 미답변 질문이 남아 갱신을 보류합니다. 권장안을 자동 채택하지 않습니다.</p>:<>
