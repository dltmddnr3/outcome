...
export function OutcomeDashboard({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  return <section className="cn-dashboard oc-dashboard" data-project-id={project.project.id} data-current-stage-id={current.stage.id} data-selected-stage-id={selected.stage.id}>
    ...
    <section className="oc-activity-band" aria-label="현재 작업과 역할 연결">
      ...
      <div className="oc-bindings" role="group" aria-label="역할별 세션 관측">{project.bindings.map((binding) => { const live = liveBinding === binding; return <div key={binding.role} className={`oc-role-row ${binding.status}`} data-live={live ? 'true' : 'false'}><strong>{roleLabel(binding.role)}</strong><span>{entityStateLabel(binding.status)} · {freshnessLabelKo(binding.freshness)}</span>{live && <span className="oc-live-signal" aria-label="실시간 활동"><i aria-hidden="true" /><span className="oc-live-bars" aria-hidden="true"><i /><i /><i /></span><b>실시간</b></span>}</div> })}</div>
    </section>
    <section className="oc-current-flow" aria-labelledby="oc-flow-title">
      ...
      <div className="oc-flow-levels">
        ...
        <div className="oc-funnel-row oc-funnel-gate" data-closed={currentSummary.closed} data-total={currentSummary.total}><div className="oc-flow-axis" aria-hidden="true">4</div><div className="oc-flow-copy"><small>완료 조건</small><strong>현재 작업 단계 수용 기준</strong><p>{currentSummary.boundaryCopy}</p></div></div>
      </div>
    </section>
    <section className="oc-current-stage" aria-labelledby="oc-current-stage-title"><header><div><small>현재 작업 단계에서 먼저 확인할 것</small><h2 id="oc-current-stage-title">{stagePresentation(current.stage.id)[0]}</h2></div></header><p>{currentSummary.boundaryCopy}</p>{currentSummary.remaining.length ? <ol>{currentSummary.remaining.map((gate) => <li key={gate.id}><b>{gate.id}</b><span>{gatePresentation(current.stage.id, gate.id)}</span></li>)}</ol> : <p>{currentSummary.checkedCopy}</p>}</section>
    <section className="oc-stage-explorer" aria-labelledby="oc-explorer-title"><header><div><h2 id="oc-explorer-title">작업 단계 탐색</h2><p>원본 작업 단계 {allStages.length}개 · 선택은 현재 흐름을 바꾸지 않습니다.</p></div></header><div className="oc-explorer-layout"><div className="oc-stage-list" role="listbox" aria-label="원본 작업 단계">{stageGroups.map((group) => <section className="oc-stage-group" key={group.scope.id} aria-label={scopePresentation(group.scope.id)[0]}><h3>{scopePresentation(group.scope.id)[0]}</h3>{group.stages.map((stage) => { const index = allStages.findIndex((item) => item.stage.id === stage.id); const isSelected = stage.id === selected.stage.id; const isCurrent = stage.id === project.current?.stageId; return <button key={stage.id} role="option" tabIndex={isSelected ? 0 : -1} data-stage-id={stage.id} data-stage-state={stage.state} className={isSelected ? 'active' : ''} aria-selected={isSelected} aria-current={isCurrent ? 'step' : undefined} onKeyDown={(event) => selectStageByKeyboard(event, index)} onClick={() => setSelectedStageId(stage.id)}><i className={stage.state} aria-hidden="true">{stage.state === 'complete' && <Check size={12} />}</i><span><strong>{stagePresentation(stage.id)[0]}</strong><small>{entityStateLabel(stage.state)}</small></span></button> })}</section>)}</div>
        <section className="oc-detail oc-selected-detail" data-stage-state={selected.stage.state} data-exploring={exploring ? 'true' : 'false'} aria-labelledby="oc-selected-stage-title"><div className="oc-selection-status" aria-live="polite">{exploring ? <span className="oc-exploration-badge">탐색 중 · 실제 현재 위치는 {stagePresentation(current.stage.id)[0]} 유지</span> : <span>현재 작업 단계 상세</span>}<h3 id="oc-selected-stage-title">{stagePresentation(selected.stage.id)[0]}</h3></div><p>{stagePresentation(selected.stage.id)[1]}</p><dl className="oc-detail-facts"><div><dt>의존성</dt><dd>{selected.stage.dependsOn.length ? selected.stage.dependsOn.map((id) => stagePresentation(id)[0]).join(' · ') : '선행 작업 단계 없음'}</dd></div><div><dt>완료 조건 확인</dt><dd>{selected.stage.gate.available ? selectedSummary.countLabel : '완료 조건 근거 없음'}</dd></div></dl><p className="oc-detail-boundary">{selectedSummary.boundaryCopy}</p><div className="oc-detail-grid"><section><h4>선택 작업 단계의 남은 완료 조건</h4>{selectedSummary.remaining.length ? <ol>{selectedSummary.remaining.map((gate) => <li key={gate.id}><b>{gate.id}</b><span>{gatePresentation(selected.stage.id, gate.id)}</span></li>)}</ol> : <p>{selectedSummary.checkedCopy}</p>}</section><section><h4>완료 조건 그룹</h4>{selected.stage.gate.groups.length ? <div className="oc-groups">{selected.stage.gate.groups.map((group) => <article key={group.code}><span><strong>{groupPresentation(group.name, group.code)}</strong><small>코드 {group.code}</small></span><b>{group.closed}/{group.total}</b></article>)}</div> : <p>그룹 근거 없음</p>}</section></div></section>
      </div></section>
    <details className="oc-technical"><summary><span>기술 증거</span><small>빌드·역할 연결·GitHub·근거 축</small></summary><div className="oc-technical-content"><div className="oc-build" aria-label="제공 중인 고정 빌드"><small>제공 중인 고정 빌드 · {data.build.repository}/{data.build.ref}</small><strong>커밋 {data.build.commit ?? '근거 없음'} · 트리 {data.build.tree ?? '근거 없음'}</strong><span>에셋 {data.build.asset ?? '근거 없음'} · 실시간 현재 작업은 빌드에 고정되지 않음</span></div><section className="oc-binding-evidence" aria-label="역할 연결 기술 증거"><h3>역할 연결 근거</h3>{project.bindings.map((binding) => <div key={binding.role}><strong>{roleLabel(binding.role)}</strong><span>{binding.stageId ?? '작업 단계 연결 없음'} · 이력 {binding.historyCount} · 관측 {compactTime(binding.observedAt)}</span></div>)}</section><section className="oc-github" aria-label="GitHub 전달 근거 연결"><header><div><small>선택 연결 근거 · GitHub</small><strong>{github.adopted ? 'GitHub 연결 채택' : 'GitHub 연결 미채택'}</strong></div><span className={github.state}>{entityStateLabel(github.state)}</span></header><div>{githubEvidenceItems(github).map((item) => <article key={item.label} className={item.state}><small>{item.label}</small><strong>{item.value}</strong><span>{entityStateLabel(item.state)}</span></article>)}</div><p>완료 판정 권한 없음 · GitHub 활동은 완료 조건 충족이나 Cherry 승인이 아닙니다.</p></section><div className="oc-axes" aria-label="선택 작업 단계 근거 축"><Axis label="구현" value={selected.stage.axes.implementation} /><Axis label="테스트" value={selected.stage.axes.test} /><Axis label="증거 확정" value={selected.stage.axes.evidence} /><Axis label="변화 관측" value={project.now.status} /></div></div></details>
  </section>
}
