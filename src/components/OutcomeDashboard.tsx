...
export function OutcomeDashboard({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  return <section className="cn-dashboard oc-dashboard" data-project-id={project.project.id} data-current-stage-id={current.stage.id} data-selected-stage-id={selected.stage.id}>
    ...
    <section className={`oc-hero ${project.status}`} aria-labelledby="oc-project-title">{gateEvidence.available && <span className="oc-hero-fill" style={{ transform: `scaleX(${gateEvidence.scale})` }} aria-hidden="true" />}<div className="oc-hero-title"><Layers3 size={22} aria-hidden="true" /><div><small>프로젝트 식별자 · {project.project.id}</small><h2 id="oc-project-title">{hero.name}</h2><p>{hero.outcome}</p></div></div><div className="oc-hero-orientation"><div><small>현재 위치</small><strong>{hero.current}</strong></div><ChevronRight size={17} aria-hidden="true" /><div><small>다음 경계</small><strong>{hero.next}</strong></div></div><div className="oc-hero-meta"><span className={`oc-source ${project.status}`}><strong>{sourceStateLabel(project.status)}</strong><small>원본 관측 {hero.freshness}</small></span><button className="cn-refresh" onClick={() => void load()} aria-label="원본 묶음 새로고침"><RefreshCw size={16} /></button></div><div className="oc-hero-gate" data-gate-fill={gateEvidence.available ? 'available' : 'unavailable'}><small>현재 작업 단계 완료 조건 근거</small><strong>{gateEvidence.available ? `${gateEvidence.closed}/${gateEvidence.total} 체크됨` : '완료 조건 근거 없음'}</strong><span>프로젝트 전체 진행률이 아닙니다.</span></div>{project.status !== 'valid' && <div className={`oc-warning ${project.status}`} role="status"><strong>{sourceStateLabel(project.status)}</strong><span>원본 묶음의 참조와 식별자를 다시 확인하세요.</span></div>}</section>
    <section className="oc-now-summary"><small>현재 작업 · 실시간 · 빌드에 고정되지 않음</small><strong>{activityLabelKo(project.now.activity) ?? `구현 ${entityStateLabel(project.now.status)}`}</strong><span>{sourceLabelKo(project.now.source)} · {compactTime(project.now.observedAt)} · 세션 활동은 진행률이 아닙니다.</span></section>
