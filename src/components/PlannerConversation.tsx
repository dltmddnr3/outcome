import { createContext, useCallback, useContext, useEffect, useRef, useState, type FormEvent, type KeyboardEvent } from 'react'
import { startChatRefresh } from '../lib/chat-refresh'
import { fetchPrivateChatTimeline, submitPrivatePlannerMessage, type PrivateChatEvent, type PrivateModelV2Event } from '../lib/api'

type ConversationSession = { sessionCredential?: string; getSessionCredential?: () => Promise<string | null> } | null
export const PlannerConversationSession = createContext<ConversationSession>(null)
export async function resolveConversationCredential(session: ConversationSession) {
  if (!session?.getSessionCredential) return session?.sessionCredential
  const token = await session.getSessionCredential()
...
const roleCopy: Record<Role, string> = { planner: '플래너', builder: '구현', ux_product_qa: '사용성·품질 검증', release_audit: '출시 전 점검' }
const filterCopy: Record<RoleChatFilter, string> = { 전체: '전체', Planner: '플래너', Builder: '구현', 'UX & Product QA': '사용성·품질 검증', 'Release Audit': '출시 전 점검' }
const chatKindCopy: Record<PrivateChatEvent['kind'], string> = { user_message: '내 메시지', assistant_message: '답변', commentary: '작업 안내', plan: '작업 계획', tool_call: '도구 요청', tool_result: '도구 결과', file_change: '파일 변경', diff: '변경 내용', test_result: '검증 결과', approval_request: '승인 요청', waiting_user: '답변 요청', error: '오류', connection: '연결 상태' }
const chatStateCopy: Record<PrivateChatEvent['state'], string> = { queued: '대기 중', responding: '응답 중', tool_running: '도구 실행 중', verifying: '검증 중', waiting_approval: '승인 대기', waiting_user: '답변 대기', completed: '처리됨', failed: '실패', cancelled: '취소됨', reconnecting: '다시 연결 중' }
...
const fixturePresentation: Record<RoleChatFixtureState, { label: string; detail: string; writable: boolean }> = {
  ready: { label: '준비됨', detail: '플래너에게 메시지를 보낼 준비가 되었습니다.', writable: true }, streaming: { label: '응답 수신 중', detail: '플래너 응답을 받고 있습니다.', writable: false }, 'tool-running': { label: '도구 실행 중', detail: '플래너가 도구 결과를 기다리고 있습니다.', writable: false }, 'waiting-approval': { label: '승인 대기', detail: '사용자의 명시적 승인이 필요합니다.', writable: false }, 'offline-reconnecting': { label: '오프라인 · 재연결 중', detail: '연결이 복구될 때까지 전송하지 않습니다.', writable: false }, 'permission-absent': { label: '권한 없음', detail: '플래너 전송 권한이 없습니다.', writable: false }, 'unbound-stale': { label: '연결 없음 · 관측 오래됨', detail: '현재 플래너 연결을 확인할 수 없습니다.', writable: false }, delivery_unknown: { label: '전달 상태 확인 불가', detail: '전달 근거를 확인하기 전에는 자동 재전송하지 않습니다.', writable: false },
}
...
export function validatePrivateTimeline(events: PrivateChatEvent[]) {
  const ids = new Set<string>(); let prior = 0, priorTime = ''
  for (const event of events) {
    const time = Date.parse(event.observed_at)
    if (event.sequence !== prior + 1 || ids.has(event.event_id) || !chatKinds.has(event.kind) || !chatStates.has(event.state) || !Number.isFinite(time) || new Date(time).toISOString() !== event.observed_at || priorTime && event.observed_at < priorTime) throw new Error('timeline_conflict')
    const text = event.payload?.private_content?.text
    if (event.kind === 'user_message' && (event.state !== 'queued' || typeof text !== 'string' || !Object.hasOwn(deliveryCopy, event.delivery))) throw new Error('timeline_conflict')
    if (text !== undefined && (typeof text !== 'string' || !text.trim() || !['user_message', 'assistant_message'].includes(event.kind))) throw new Error('timeline_conflict')
    ids.add(event.event_id); prior = event.sequence; priorTime = event.observed_at
  }
  return events
}
...
export function PlannerConversation({ events, plannerBound = false, onSend, fixtureState, fixtureTimeline, initialFilter = '전체' }: { events: PrivateModelV2Event[]; plannerBound?: boolean; onSend?: (message: string) => void; fixtureState?: RoleChatFixtureState; fixtureTimeline?: PrivateChatEvent[]; initialFilter?: RoleChatFilter }) {
  ...
  const credential = () => resolveConversationCredential(session)
  const refresh = useCallback(async () => {
    const generation = authGeneration.current, request = ++refreshRequest.current
    const current = () => generation === authGeneration.current && request === refreshRequest.current
    try {
      const token = await resolveConversationCredential(session)
      if (!current()) return
      const value = await fetchPrivateChatTimeline('outcome', 0, token)
      if (!current()) return
      setTimeline(validatePrivateTimeline(value.events)); setCsrf(value.csrf); setAvailability('ready')
    } catch (error) {
      if (!current()) return
      setTimeline([]); setCsrf(null); setAvailability(error instanceof Error && error.message === 'timeline_conflict' ? 'conflict' : 'chat_unavailable')
    }
  }, [session?.sessionCredential, session?.getSessionCredential])
  useEffect(() => {
    authGeneration.current++; refreshRequest.current++; pending.current = false; submission.current = null
    if (fixtureState || onSend) return
    setTimeline([]); setCsrf(null); setDraft(''); setDelivery(null); setRetryAvailable(false); setAvailability('loading')
    const stop = startChatRefresh(refresh)
    return () => { stop(); authGeneration.current++; refreshRequest.current++ }
  ...
  const submit = async () => {
    ...
    const generation = authGeneration.current, key = submission.current.key
    try {
  ...
  return <section className="planner-conversation" aria-labelledby="planner-conversation-title" data-observed-events={events.length} data-chat-availability={availability} data-role-chat-state={fixtureState} data-fixture-boundary={fixture || fixtureTimeline ? 'non-production' : undefined}>
    <header><div><span>프로젝트 대화</span><h2 id="planner-conversation-title">플래너와 대화</h2></div><strong>{writable ? '메시지 전송 가능' : '읽기 전용'}</strong></header>
    {writable && <p className="planner-conversation__boundary">전송 가능 표시는 실행기의 작동 상태를 보장하지 않습니다. 접수·답변 여부는 각 메시지의 확인 결과를 확인해 주세요.</p>}
    <nav className="planner-conversation__filters" aria-label="역할 대화 필터">{roleChatFilters.map((label) => <button key={label} data-role-filter={label} type="button" aria-pressed={filter === label} onClick={() => setFilter(label)}>{filterCopy[label]}</button>)}</nav>
    ...
    {eligibleLens && timeline.length > 0 && <ol className="planner-conversation__messages" aria-label="서버에서 확인된 메시지">{timeline.map((event) => <li key={event.event_id} data-sequence={event.sequence}><div><strong>{event.kind === 'user_message' ? '나' : '플래너'}</strong><time dateTime={event.observed_at}>{event.observed_at}</time></div>{event.payload.private_content && <p>{event.payload.private_content.text}</p>}<span>{event.kind === 'user_message' ? hasCorrelatedPlannerAnswer(timeline, event.correlation_id) ? '플래너 답변 확인됨' : deliveryCopy[event.delivery] : `${chatKindCopy[event.kind]} · ${chatStateCopy[event.state]}`}</span></li>)}</ol>}
    {visible.length === 0 ? <p className="planner-conversation__empty" role="status">아직 확인된 작업 기록이 없습니다</p> : <ol>{visible.map((event) => <li key={event.id} data-event-id={event.id} data-event-sequence={event.sequence} data-event-role={event.role} data-event-type={event.type} data-event-status={event.status}><details className="planner-conversation__activity" open={['blocked', 'failed', 'rejected', 'safe_hold', 'delivery_unknown'].includes(event.status)}><summary><strong>{roleCopy[event.role]} · {typeCopy[event.type]}</strong><span>{statusCopy[event.status]}</span></summary><time dateTime={event.observedAt}>{event.observedAt}</time><p>{event.summary}</p>{event.role === 'release_audit' && <small>완료 판정 권한 없음.</small>}{event.status === 'delivery_unknown' && <small>전달 근거를 확인하기 전에는 자동 재전송하지 않습니다.</small>}</details></li>)}</ol>}
    {writable && <form className="planner-conversation__composer" data-planner-composer="true" onSubmit={onSubmit}><label htmlFor="planner-message">플래너에게 메시지</label><textarea ref={textarea} id="planner-message" rows={4} value={draft} aria-describedby="planner-message-help planner-message-status" onKeyDown={onKeyDown} onChange={(event) => setDraft((current) => boundedComposerDraft(current, event.target.value))} /><div id="planner-message-help"><span>⌘/Ctrl + Enter로 보내기 · Enter는 줄바꿈</span><span className={overLimit ? 'is-error' : ''}>{[...draft].length} / 4000</span></div>{sensitive && <p role="alert">민감할 수 있는 값이 감지되었습니다. 실제 자격 증명은 제거해 주세요.</p>}<div className="planner-conversation__actions"><button type="submit" data-touch-target="44" disabled={disabled}>메시지 보내기</button>{retryAvailable && <button type="button" data-touch-target="44" onClick={() => void submit()}>수동으로 다시 시도</button>}{!injectedWritable && <button type="button" data-touch-target="44" onClick={() => void refresh()}>새로고침</button>}</div><p id="planner-message-status" aria-live="polite">{delivery ? deliveryCopy[delivery] : availability === 'conflict' ? '대화 순서를 확인할 수 없습니다' : availability === 'chat_unavailable' ? '대화를 사용할 수 없습니다' : ''}</p></form>}
  </section>
}
