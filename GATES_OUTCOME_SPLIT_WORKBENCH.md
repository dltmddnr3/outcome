...
현재 결과 지도를 오른쪽 작업 영역으로 이동하고, 왼쪽에는 향후 세션 채팅을 위한 정직한 준비 영역을 둔다. 결과 지도는 `Phase → Scope → Stage` 탐색에 집중하며 선택 Stage의 Gate 완료 조건은 세 열 아래의 접기 가능한 상세 영역에서 보여준다.
...
- [x] 데스크톱에서 왼쪽 채팅 준비 영역과 오른쪽 결과 지도가 한 작업대 안에 나란히 배치된다.
  EVIDENCE: 1440×900 캡처에서 220px 채팅 열과 3열 결과 지도가 같은 작업대 안에 나란히 표시됨.
...
- [x] 태블릿·모바일에서는 결과 지도의 가용 폭을 해치지 않도록 채팅 준비 영역이 얇은 접기 행으로 바뀐다.
  EVIDENCE: 1179px 이하 단일 열 전환과 48px summary를 1024×768, 390×844, 360×800에서 검증함.

### W2 · 3단 탐색

- [x] 결과 지도의 가로 탐색 열은 Phase, Scope, Stage 세 개뿐이다.
...
- [x] 선택 Stage의 Gate 완료 조건은 세 탐색 열 아래 전체 폭에 배치된다.
  EVIDENCE: gateDetailsTruth가 Gate가 .oc-map-columns 밖이면서 결과 지도 전체 폭인지 검증함.
...
- 구조: `.oc-workbench` 안에 제어 없는 `세션 채팅 · 연결 준비 중` details와 `.oc-map-workspace`를 배치했다. 결과 지도는 `data-column=1..3`만 유지하며 Gate details는 열 밖 전체 폭에 있다.
