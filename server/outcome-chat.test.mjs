...
test('Planner answer body survives snapshot validation and repository reload', () => {
  const event = { event_id: 'event-0000000000000001', sequence: 1, observed_at: '2026-09-08T00:00:00.000Z', kind: 'assistant_message', state: 'completed', correlation_id: 'message-0000000000000001', payload: { private_content: { text: '확인된 답변입니다.' } } }
  const snapshot = { schema_version: 1, streams: [{ project_id: 'outcome', role: 'planner', binding_version: 7, events: [event] }], idempotency: [] }
  const validated = validateChatSnapshot(snapshot)
  const repository = createInMemoryChatRepository({ snapshot: validated })
  assert.deepEqual(repository.timeline({ project_id: 'outcome', binding_version: 7, after_sequence: 0 })[0].payload, event.payload)
  event.payload.private_content.text = 'token=fixture-prohibited-value'
  assert.throws(() => validateChatSnapshot(snapshot), /invalid_snapshot/)
})
