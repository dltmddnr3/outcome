...
export function parseGateContract(markdown) {
  ...
  for (const line of markdown.split('\n')) {
    ...
    if (!gate) continue
    if (sourceGroup && sourceGroupCode === null) sourceGroupCode = gate[1]
    let group = groups.find((item) => item.id === gate[1])
    if (!group) { group = { id: gate[1], label: sourceGroupCode === gate[1] ? sourceGroup : gate[1], total: 0, gates: [] }; groups.push(group) }
    group.total += 1
...
export function parseProgressSnapshots(text) {
  const latest = (pattern) => { const matches = [...text.matchAll(pattern)]; return matches.length ? Number(matches.at(-1)[1]) : null }
  return { implemented: latest(/implemented:\s*(\d+)\/(\d+)/gi), tested: latest(/tested:\s*(\d+)\/(\d+)/gi), evidenceClosed: latest(/evidence(?:-closed| closed):\s*(\d+)\/(\d+)/gi) }
}
...
export function buildCherryNoteDashboard({ root = DEFAULT_ROOT, rolloutPath = DEFAULT_ROLLOUT, now = new Date(), rolloutText, gateText, scorecardText, process, sourceObservedAt } = {}) {
  ...
  return {
    ...
    progress: { total, implemented: total == null ? null : clamp(progress.implemented, 0, total), tested: total == null ? null : clamp(progress.tested, 0, total), evidenceClosed: total == null ? null : clamp(progress.evidenceClosed, 0, total), percentages: { implemented: percent(progress.implemented, total), tested: percent(progress.tested, total), evidenceClosed: percent(progress.evidenceClosed, total) } },
