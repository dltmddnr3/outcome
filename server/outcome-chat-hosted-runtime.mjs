...
export function createOutcomeChatHostedRuntimeFactory({environment=process.env,driverLoader=()=>import('pg'),repositoryFactory=createOutcomeChatPostgresRepository,rateLimiterFactory=createOutcomeChatRateLimiter,rateLimitOptions={}}={}) {
  const sendEnabled = read(environment, 'OUTCOME_CHAT_SEND_ENABLED') === '1'
  ...
  return async ({accountRuntime,allowedOrigin}={}) => {
    ...
    try {
      ...
      return Object.freeze({repository,bindingVersion:configuration.bindingVersion,allowedOrigin,csrfSecret:value.csrfSecret,sendEnabled,createService:(workspaceId)=>createOutcomeChatHostedService({repository,bindingVersion:configuration.bindingVersion,workspaceId,projectId:'outcome'}),rateLimit:(input)=>limiter.check(input)})
    } catch { return null }
  }
}
