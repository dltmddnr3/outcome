...
export function renderConfirmedDestinationPackage({projectId,serializedSnapshot}={}){
 try{
  ...
  if(![1,2].includes(snapshot?.schemaVersion)||snapshot.completionAuthority!==false||snapshot.executionAuthority!==false||!Number.isSafeInteger(snapshot.intakeRevision)||snapshot.intakeRevision<1)fail()
  const document=parseDestinationDraft(JSON.stringify(snapshot.document))
  const fileInput=snapshot.schemaVersion===2
  let context
  if(fileInput){
   if(snapshot.inputKind!=='file_import'||document.mode!=='file_import'||snapshot.contextRevision!==snapshot.intakeRevision||document.unknowns.length||fields.some(key=>!document.answers[key]))fail()
  }else{
  context=parseDiscoveryContext(JSON.stringify(snapshot.context))
