...
export function createPlannerResponseConsumer({repository,queueAdapter,scope}={}) {
  ...
  return Object.freeze({async runOnce(){
    ...
    used=true
    let pending
    try {pending=await repository.pendingPlannerResponses(target)}catch{return {outcome:'unavailable'}}
