...
const validateEvent = (value) => {
  ...
  if (!row.payload || typeof row.payload !== 'object' || types.isProxy(row.payload)) fail('invalid_snapshot')
  const hasText = row.kind === 'user_message' || (row.kind === 'assistant_message' && Object.hasOwn(row.payload, 'private_content'))
  const payload = exact(row.payload, hasText ? ['private_content'] : [], 'invalid_snapshot')
  const privateContent = hasText ? exact(payload.private_content, ['text'], 'invalid_snapshot') : null
  return {
    ...
    payload: hasText ? { private_content: { text: safeText(privateContent.text, 4_000, 'invalid_snapshot') } } : {},
  }
}
