...
const binding={project_id:'outcome',role:'planner',status:'active',freshness:'fresh',destination:'opaque-destination',binding_version:1}
...
test('reuses existing Planner binding and sends only opaque analysis reference',async()=>{
 ...
 assert.equal(result.readReceipt.message,sent[0].message)
 assert.equal(result.readReceipt.bindingVersion,1)
