...
test('hosted driver stays unloaded when disabled and send requires exact snapshotted opt-in', async () => {
  let loads = 0
  const context = { accountRuntime:{ service:{ resolveBridgeAuthority() {} } }, allowedOrigin:'https://preview.invalid' }
  const driverLoader = async () => { loads++; return { Pool: class { connect() {} } } }
  assert.equal(await createOutcomeChatHostedRuntimeFactory({ environment:{}, driverLoader })(context), null)
  assert.equal(loads, 0)
  for (const flag of [undefined, '0', 'true', '1']) {
    const env = { ...environment(), ...(flag === undefined ? {} : { OUTCOME_CHAT_SEND_ENABLED:flag }) }
    const factory = createOutcomeChatHostedRuntimeFactory({ environment:env, driverLoader, repositoryFactory:() => ({ reserve() {}, timeline() {} }) })
    env.OUTCOME_CHAT_SEND_ENABLED = flag === '1' ? '0' : '1'
    assert.equal((await factory(context)).sendEnabled, flag === '1')
  }
})
