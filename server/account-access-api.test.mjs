...
test('the one decision route authenticates, enforces origin CSRF and freshness, and binds a server-owned blocker', async () => {
  ...
  try {
    ...
    assert.deepEqual((await history.json()).decisions,[{receipt:first.body,target:{projectId:'outcome',eventId:'event-builder-blocked',sequence:7},withdrawn:false}])
...
test('decision route rejects unauthenticated callers before accessing its runtime', async () => {
  let touches = 0
  const decisionRuntime = new Proxy({}, { get() { touches += 1; throw new Error('must not inspect') } })
  for (const token of [undefined, 'invalid']) {
    assert.deepEqual(await handlePrivateAccessRequest({ method: 'POST', pathname: '/api/private/decisions', token, service, decisionRuntime }), { status: 401, body: { error: 'authentication_required' } })
  }
  assert.equal(touches, 0)
})

test('decision route rejects forged actor workspace and target authority fields before recording', async () => {
  let writes = 0
  const decisionRuntime = { service: { record: async () => { writes += 1 } }, allowedOrigin: 'https://private.example', csrfSecret: 'csrf-secret-value' }
  const base = { method: 'POST', pathname: '/api/private/decisions', token: 'valid', service, decisionRuntime, origin: 'https://private.example', headers: { 'content-type': 'application/json', 'x-outcome-csrf': 'csrf-secret-value' } }
  for (const key of ['actorSubject', 'workspaceId', 'sourceRevision', 'completionAuthority', 'currentSourcePrecondition', 'target']) {
    assert.deepEqual(await handlePrivateAccessRequest({ ...base, body: { projectId: 'outcome', [key]: 'forged' } }), { status: 400, body: { error: 'invalid_request' } })
  }
