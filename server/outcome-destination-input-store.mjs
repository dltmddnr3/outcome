...
const validInput=input=>{
 const questions=input?.purpose==='destination_questions_only'
 const keys=questions?['requestId','draftId','contextRevision','contextDigest','serializedContext','purpose','executionAuthority']:['requestId','draftId','draftRevision','documentDigest','serializedDocument','purpose','executionAuthority']
 ...
 if(input.executionAuthority!==false)fail()
 if(questions){
  const context=parseDiscoveryContext(input.serializedContext)
  if(!Number.isSafeInteger(input.contextRevision)||input.contextRevision<1||context.revision!==input.contextRevision||discoveryContextDigest(context)!==input.contextDigest)fail()
 }else if(input.purpose!=='destination_analysis_only'||!Number.isSafeInteger(input.draftRevision)||input.draftRevision<1||destinationDocumentDigest(parseDestinationDraft(input.serializedDocument))!==input.documentDigest)fail()
 return input
}
export function createDestinationInputStore({directory,scopeKey}={}) {
 ...
 const read=async reference=>{
  ...
  try{
   ...
   if(!stat.isFile()||(stat.mode&0o777)!==0o600||stat.uid!==process.getuid()||stat.nlink!==1||stat.size>16777216)fail()
 ...
 return Object.freeze({
  ...
  async publish(input){
   try{
    ...
    if(bytes.length>(input.purpose==='destination_questions_only'?16777216:524288))fail()
    ...
    return verified.purpose==='destination_questions_only'?{state:'ready',reference,requestId:verified.requestId,contextRevision:verified.contextRevision,contextDigest:verified.contextDigest}:{state:'ready',reference,requestId:verified.requestId,draftRevision:verified.draftRevision,documentDigest:verified.documentDigest}
   }catch{fail()}
  },
 })
}
