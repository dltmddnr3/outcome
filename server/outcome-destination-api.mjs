...
export async function handleDestinationDraftRequest({method='GET',pathname='',token,identityService,runtime,headers={},body}={}){
 const match=/^\/api\/private\/destination\/(drafts|analysis|discovery|questions|question-requests|review)\/([a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12})$/.exec(pathname)
 if(!match)return result(404,'not_found')
 const analysis=match[1]==='analysis',discovery=match[1]==='discovery',questions=match[1]==='questions',questionRequest=match[1]==='question-requests',writeMethod=analysis||questionRequest?'POST':'PUT'
 const review=match[1]==='review'
 if((questions||review)&&method!=='GET')return result(405,'method_not_allowed')
 ...
 const repository=analysis?runtime?.analysisRepository:questionRequest?runtime?.questionRequests:questions||review?runtime?.questionRepository:discovery?runtime?.discoveryRepository:runtime?.repository
 if(!repository||typeof repository.load!=='function'||review&&typeof repository.review!=='function'||!questions&&!review&&typeof repository[analysis||questionRequest?'enqueue':'save']!=='function')return result(503,'destination_unavailable')
 ...
 try{
  ...
  const field=analysis?'analysis':questionRequest?'questionRequest':questions?'questions':discovery?'discovery':'draft'
  ...
  if(typeof body!=='string'||Buffer.byteLength(body)>(analysis||questionRequest?4096:discovery?4194304:262144))return result(400,'invalid_request')
  let parsed;try{parsed=JSON.parse(body)}catch{return result(400,'invalid_request')}
  const input=data(parsed,analysis?['draftId','draftRevision','documentDigest']:questionRequest?['contextDigest']:discovery?['requestId','expectedRevision','intakeRevision','context']:['requestId','expectedRevision','document'])
  ...
  const value=await repository[analysis||questionRequest?'enqueue':'save']({...scope,...input})
  return {status:analysis||questionRequest?202:200,body:{[field]:value,completionAuthority:false}}
 }catch(error){
