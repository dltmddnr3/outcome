...
export async function handleDestinationDraftRequest({method='GET',pathname='',token,identityService,runtime,headers={},body}={}){
 const match=/^\/api\/private\/destination\/(drafts|analysis|discovery|questions|question-requests)\/([a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12})$/.exec(pathname)
 ...
 const repository=analysis?runtime?.analysisRepository:questionRequest?runtime?.questionRequests:questions?runtime?.questionRepository:discovery?runtime?.discoveryRepository:runtime?.repository
 if(!repository||typeof repository.load!=='function'||!questions&&typeof repository[analysis||questionRequest?'enqueue':'save']!=='function')return result(503,'destination_unavailable')
