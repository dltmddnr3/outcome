...
export function workQueueEnvelope(input){
  if(typeof input?.scopeJson!=='string'||Buffer.byteLength(input.scopeJson)>2048
    ||!hash(input.reservationDigest,64)||!hash(input.authorityRef,64)||!hash(input.candidateCommit,40)||!hash(input.candidateTree,40)
    ||!['implementing','qa_verifying','release_verifying'].includes(input.action))throw Error('invalid_work_request')
  const scope=JSON.parse(input.scopeJson)
  return {message:JSON.stringify({type:'outcome-stage-request',schemaVersion:1,scope,
    ...
    correlation_id:`message-${input.reservationDigest.slice(0,16)}`}
}
...
export function createWorkQueueDispatch(queue){
  return async(input,{signal}={})=>{
    try{
      ...
      const {message,correlation_id}=workQueueEnvelope(input)
