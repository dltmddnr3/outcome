...
export function createDiscoveryQuestionDispatch({queueAdapter,publishInput}={}) {
 ...
 return async input=>{
  ...
  try{
   ...
   const message=discoveryQuestionMessage(input,publication.reference)
...
// Deterministic source identity shared by dispatch and collection-only recovery.
// Constructing this text is never a transport acknowledgement.
export function discoveryQuestionMessage(input,reference){
 const context=parseDiscoveryContext(input.serializedContext)
 if(input.purpose!=='destination_questions_only'||input.executionAuthority!==false||context.revision!==input.contextRevision||discoveryContextDigest(context)!==input.contextDigest||typeof input.requestId!=='string'||!/^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/.test(input.requestId)||typeof reference!=='string'||!/^analysis-[a-f0-9]{64}$/.test(reference))throw Error('discovery_source_unavailable')
 return [
    'OUTCOME 목적지 발견의 다음 질문 요청입니다. 실행·배포·수용 권한은 없습니다.',
    `requestId=${input.requestId}`,`contextDigest=${input.contextDigest}`,`contextRevision=${input.contextRevision}`,`reference=${reference}`,
    `설정된 OUTCOME 작업 디렉터리에서 node .outcome-runtime/read-destination-input.mjs ${reference} 로 비공개 맥락을 읽으세요. reader가 비활성/실패하면 설정·권한을 변경하거나 맥락을 추정하지 말고 보류하세요.`,
