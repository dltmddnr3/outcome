...
export function createDesktopObservationReader({ socketPath, timeoutMs = 3000, maxBytes = 8_000_000, now = Date.now, onDiagnostic = () => {} } = {}) {
  if (typeof socketPath !== 'string' || !isAbsolute(socketPath) || !Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 10000
    || !Number.isSafeInteger(maxBytes) || maxBytes < 1 || maxBytes > 8_000_000 || typeof now !== 'function' || typeof onDiagnostic !== 'function') throw Error('reader_configuration_invalid')
  const report = (phase, reason, bytes = 0) => { try { onDiagnostic(Object.freeze({ phase, reason, bytes })) } catch {} }
  return threadId => new Promise(resolve => {
    if (!uuid(threadId)) { report('preflight', 'identity_invalid'); resolve(null); return }
    try {
      ...
      if (uid === undefined || !socket.isSocket() || socket.uid !== uid || (socket.mode & 0o777) !== 0o600
        || !parent.isDirectory() || parent.uid !== uid || (parent.mode & 0o777) !== 0o700) { report('preflight', 'socket_boundary_invalid'); resolve(null); return }
    } catch { report('preflight', 'socket_unavailable'); resolve(null); return }
    ...
    const finish = (value, reason = 'protocol_invalid') => {
      if (done) return
      done = true; clearTimeout(timer); buffer = Buffer.alloc(0)
      report(following ? 'snapshot' : client ? 'owner_discovery' : 'initialize', value ? 'observed' : reason, bytes)
    ...
    try {
      ...
      socket.on('error', () => finish(null, 'socket_error')); socket.on('end', () => finish(null, 'socket_closed')); socket.on('close', () => finish(null, 'socket_closed'))
      ...
      socket.on('data', chunk => {
        if (done) return
        try {
          bytes += chunk.length; if (bytes > maxBytes) { finish(null, 'payload_limit'); return }
          buffer = Buffer.concat([buffer, chunk])
          while (!done && buffer.length >= 4) {
            const length = buffer.readUInt32LE(0)
            if (!length || length > maxBytes) { finish(null, length ? 'payload_limit' : 'frame_invalid'); return }
            ...
            if (message.type === 'response' && message.requestId === initId && !client) {
              if (message.method !== 'initialize' || message.resultType !== 'success' || !uuid(message.result?.clientId)) { finish(null, 'initialize_rejected'); return }
              ...
              if (message.method !== 'thread-owner-discovery' || message.resultType !== 'success' || !uuid(message.handledByClientId)
                || message.handledByClientId === client || message.result?.supportsUntrustedAppInput !== true) { finish(null, 'owner_unverified'); return }
              ...
              if (change?.type !== 'snapshot' || !Number.isSafeInteger(change.revision) || change.revision < 0 || change.conversationState?.id !== threadId) { finish(null, 'snapshot_invalid'); return }
              ...
              if (projection.reason === 'source_unavailable') { finish(null, 'runtime_status_invalid'); return }
