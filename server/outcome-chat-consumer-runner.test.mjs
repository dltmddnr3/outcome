...
test('response-only mode requires its own opt-in and never constructs dispatch runtime',async()=>{
  let sends=0,reads=0,loads=0
  const env=environment({OUTCOME_CHAT_RESPONSES_ENABLED:'1',OUTCOME_CHAT_WORKSPACE_ID:'workspace-one',OUTCOME_CHAT_PLANNER_BINDING_VERSION:'3',OUTCOME_CODEX_EXECUTABLE:'/synthetic/codex'})
  const options={environment:env,argv:['--responses'],driverLoader:async()=>{loads++;return {Pool:class {async end(){}}}},transactionFactory:()=>async()=>{},repositoryFactory:()=>({}),queueAdapterFactory:value=>{assert.equal(value.codexExecutable,'/synthetic/codex');return {}},runtimeFactory:()=>{sends++;throw Error('no send')},responseRuntimeFactory:({scope})=>{assert.deepEqual(scope,{workspace_id:'workspace-one',project_id:'outcome',binding_version:3});return {runOnce:async()=>{reads++;return {outcome:'checked',stored:1,pending:0,unavailable:0}}}},write:()=>{}}
  assert.equal(await runOutcomeChatConsumerOnce(options),0)
  assert.equal(sends,0);assert.equal(reads,1);assert.equal(loads,1)
  assert.equal(await runOutcomeChatConsumerOnce({...options,environment:{...env,OUTCOME_CHAT_RESPONSES_ENABLED:'0'}}),64)
