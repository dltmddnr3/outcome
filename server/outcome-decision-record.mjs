...
export function createDecisionRecordService({ store, now = Date.now, identifier = randomUUID } = {}) {
  ...
  return Object.freeze({
    async record(value) {
      ...
      const requestDigest = hash(stable({ operation: 'record', actorSubject: input.actorSubject, workspaceId: input.workspaceId, decision: input.decision, eventId: input.target.eventId, nonce: input.nonce, projectId: input.target.projectId, rejectionReason: input.rejectionReason, sequence: input.target.sequence, sourceRevision: input.target.sourceRevision, targetState: input.target.state, targetRole: input.target.role, targetStatus: input.target.status, sourcePrecondition: input.sourcePrecondition, supersedesId: input.supersedesId }))
    ...
    async withdraw(value) {
      ...
      const input = checked.input
      const nonceDigest = hash(`${input.actorSubject}\n${input.workspaceId}\n${input.nonce}`)
      const requestDigest = hash(stable({ actorSubject: input.actorSubject, workspaceId: input.workspaceId, decisionId: input.decisionId, nonce: input.nonce, operation: 'withdraw', projectId: input.projectId, sourcePrecondition: input.sourcePrecondition }))
      return transact(async (tx) => {
        const exact = await tx.replayByRequestDigest(requestDigest)
