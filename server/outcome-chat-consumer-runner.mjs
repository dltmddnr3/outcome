...
export const OUTCOME_CHAT_CONSUMER_ENV = Object.freeze({
  ...
  responsesEnabled: 'OUTCOME_CHAT_RESPONSES_ENABLED',
  workspaceId: 'OUTCOME_CHAT_WORKSPACE_ID',
  bindingVersion: 'OUTCOME_CHAT_PLANNER_BINDING_VERSION',
  executable: 'OUTCOME_CODEX_EXECUTABLE',
})
...
const TERMINALS = Object.freeze({
  ...
  responses: [0, 'OUTCOME_CHAT_RESPONSES_CHECKED\n'],
})
...
export function readOutcomeChatConsumerConfiguration(environment = {}, { pathStat = lstatSync } = {}) {
  ...
  try {
    ...
    const executable = read(OUTCOME_CHAT_CONSUMER_ENV.executable,true) || 'codex'
    if (!(executable==='codex'||isAbsolute(executable)) || /[\u0000-\u001f\u007f]/.test(executable)) return disabled()
    const responsesEnabled = read(OUTCOME_CHAT_CONSUMER_ENV.responsesEnabled,true)==='1'
    const workspaceId = read(OUTCOME_CHAT_CONSUMER_ENV.workspaceId,true)
    const bindingVersion = integer(read(OUTCOME_CHAT_CONSUMER_ENV.bindingVersion,true),1,2147483647)
    return { enabled: true, databaseUrl, databaseCaPem, consumerId, registryPath, leaseMs, timeoutMs, executable, responsesEnabled, workspaceId, bindingVersion }
