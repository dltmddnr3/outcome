...
test('identity-only account mode exposes publishable key, verifies owner, serves exactly the sealed projects, and never mints session cookies', async () => {
  ...
  for (const headers of [{ cookie: '__session=sdk-valid' }, { authorization: 'Bearer sdk-valid' }]) {
    ...
    const outcome = workspace.body.workspace.projects.find((project) => project.project.id === 'outcome')
    assert.equal(outcome.resultView.hierarchy.id, 'outcome')
    assert.equal(outcome.resultView.hierarchy.children.every((phase) => phase.kind === 'phase'), true)
    ...
    const phase = outcome.resultView.hierarchy.children.find((node) => node.id === 'outcome-phase-5')
    const scope = phase.children.find((node) => node.id === 'outcome-phase-5-composition')
    const stage = scope.children.find((node) => node.id === 'outcome-milestone-model-v2-local-default-projection')
    assert.equal(stage.kind, 'stage')
    assert.deepEqual({ closed: stage.acceptance.closed, total: stage.acceptance.total, unmapped: stage.acceptance.unmapped }, { closed: 13, total: 13, unmapped: 0 })
    assert.deepEqual({ closed: outcome.resultView.source_projection.compatibility.closed, total: outcome.resultView.source_projection.compatibility.total, label: outcome.resultView.source_projection.compatibility.label }, { closed: 38, total: 43, label: 'compatibility' })
    assert.deepEqual({ closed: outcome.resultView.source_projection.historical[0].closed, total: outcome.resultView.source_projection.historical[0].total, label: outcome.resultView.source_projection.historical[0].label }, { closed: 5, total: 6, label: 'historical' })
