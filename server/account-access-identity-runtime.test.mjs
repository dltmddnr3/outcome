...
test('identity-only account mode exposes publishable key, verifies owner, serves exactly the sealed projects, and never mints session cookies', async () => {
  ...
  for (const headers of [{ cookie: '__session=sdk-valid' }, { authorization: 'Bearer sdk-valid' }]) {
    ...
    const phase = outcome.resultView.hierarchy.children.find((node) => node.id === 'outcome-phase-2')
    const scope = phase.children.find((node) => node.id === 'outcome-phase-2-account-service')
    const stage = scope.children.find((node) => node.id === 'outcome-stage-account-access-hosted-identity-preview')
