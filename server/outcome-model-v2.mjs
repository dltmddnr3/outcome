...
export function projectOutcomeV2({ graph: input, source_revision, expected_source_revision = source_revision, observed_at, work_items = [], attempts = [], mission_envelope = null, leases = [], verification_history = [] }) {
...
export function applyOutcomeModelV2Pilot(value, { environment = process.env, source_revision, observed_at = value.observedAt } = {}) {
...
export function createCodexRuntimeAdapter(controlPlane) {
  ...
  return frozen({ select: (work) => controlPlane.selectNext(work), startValidated: (command) => controlPlane.start(command), recordObserved: (event) => controlPlane.transition(event), recordEvidenceEvaluation: (event) => controlPlane.transition(event), projectRuntime: () => controlPlane.projectPublic(), canCommitCanonicalTransition: false })
}
