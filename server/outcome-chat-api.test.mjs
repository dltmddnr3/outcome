...
test('Planner answer text survives authenticated timeline projection', async () => {
  const event = { event_id: 'event-0000000000000001', sequence: 1, observed_at: '2026-09-08T00:00:00.000Z', kind: 'assistant_message', state: 'completed', correlation_id: 'message-0000000000000001', payload: { private_content: { text: '검증한 결과와 다음 행동입니다.' } } }
  ...
  assert.deepEqual(result.body.events[0].payload, event.payload)
})

test('Planner answer rejects credentials and hostile payloads without callback or echo', async () => {
  let traps = 0
  const trap = () => { traps++; throw new Error('private-payload') }
  const payloads = [
    { private_content: { text: 'token=fixture-prohibited-value' } },
    new Proxy({}, { getOwnPropertyDescriptor: trap, ownKeys: trap, get: trap }),
    { get private_content() { return trap() } },
    { private_content: { text: 'ordinary' }, locator: 'private' },
  ]
  for (const payload of payloads) {
    const event = { event_id: 'event-0000000000000001', sequence: 1, observed_at: '2026-09-08T00:00:00.000Z', kind: 'assistant_message', state: 'completed', correlation_id: 'message-0000000000000001', payload }
    const result = await handlePrivateChatRequest({ method: 'GET', url: '/api/private/chat/timeline?project_id=outcome&after_sequence=0', service: { timeline: async () => ({ target: { role: 'planner', binding_version: 7 }, events: [event], completion_authority: false }) }, owner })
