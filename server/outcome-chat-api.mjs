...
const eventProjection = (value) => {
  ...
  if (!row.payload || typeof row.payload !== 'object' || types.isProxy(row.payload)) fail('invalid_response')
  const hasText = kind === 'user_message' || (kind === 'assistant_message' && Object.hasOwn(row.payload, 'private_content'))
  const payload = record(row.payload, hasText ? ['private_content'] : [])
  const privateContent = hasText ? record(payload.private_content, ['text']) : null
  if (userMessage && (!DELIVERY.has(row.delivery) || !DISPATCH.has(row.dispatch_state))) fail('invalid_response')
  return { event_id: finiteEventId(row.event_id), sequence: finiteInteger(row.sequence, 1), observed_at: finiteTime(row.observed_at), kind, state, correlation_id: finiteCorrelationId(row.correlation_id), payload: hasText ? { private_content: { text: finiteText(privateContent.text) } } : {}, ...(userMessage ? { delivery: row.delivery, dispatch_state: row.dispatch_state } : {}) }
}
