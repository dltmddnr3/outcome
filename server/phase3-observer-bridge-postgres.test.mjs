...
function transactionPort(db, failPattern = null, effectiveRole = 'outcome_bridge_backend') {
...
test('tombstone transaction purges raw scope, preserves safe audit and restore denies resurrection', async () => {
  ...
  try {
    ...
    assert.deepEqual(await adapter.tombstone({ workspace_id: 'workspace-main', project_id: 'project-outcome', role: 'builder', binding_version: 1, deletion_revision: 2, deletion_receipt_digest: deletionReceipt, purge_before: AT, tombstoned_at: EXPIRES, expected_durable_revision: 1 }), { status: 'tombstoned', durable_revision: 2 })
    ...
    const tombstones = (await db.query(`select workspace_id,project_id,role,binding_version,deletion_revision,deletion_receipt_digest from outcome_private.bridge_tombstones where workspace_id='workspace-main'`)).rows
    assert.equal(tombstones.length, 1)
    const coverage = computeTombstoneCoverageDigest(tombstones)
    const manifestRef = uuid7('a')
    const restoreReceiptRef = uuid7('b')
    const missing = { restore_receipt_ref: restoreReceiptRef, manifest_ref: manifestRef, manifest_digest: digest('3'), workspace_id: 'workspace-main', project_id: 'project-outcome', role: 'builder', binding_version: 1, expected_schema_version: 1, expected_durable_revision: 2, restored_at: EXPIRES }
    await expectCode(() => adapter.verifyRestore(missing), 'restore_denied')
    const stored = await adapter.storeManifest({ manifest_ref: manifestRef, workspace_id: 'workspace-main', manifest_schema_version: 1, bridge_schema_version: 1, durable_revision: 2, tombstone_count: 1, tombstone_coverage_digest: coverage, stored_at: EXPIRES })
    await expectCode(() => adapter.storeManifest({ manifest_ref: uuid7('f'), workspace_id: 'workspace-main', manifest_schema_version: 1, bridge_schema_version: 1, durable_revision: 2, tombstone_count: 0, tombstone_coverage_digest: coverage, stored_at: EXPIRES }), 'manifest_invalid')
    ...
    await db.exec(`delete from outcome_private.bridge_tombstones where workspace_id='workspace-main'`)
    await expectCode(() => adapter.verifyRestore(restore), 'restore_denied')
    await db.exec(`insert into outcome_private.bridge_tombstones(workspace_id,project_id,role,binding_version,deletion_revision,deletion_receipt_digest,purge_before,tombstoned_at,restore_redelete_required) values ('workspace-main','project-outcome','builder',1,2,'${deletionReceipt}','${AT}','${EXPIRES}',true)`)
...
test('restore rejects caller assertion when no immutable manifest or tombstone exists', async () => {
  ...
  try {
    const adapter = createObserverBridgePostgresAdapter({ with_transaction: transactionPort(db) })
    await expectCode(() => adapter.verifyRestore({ restore_receipt_ref: uuid7('d'), manifest_ref: uuid7('e'), manifest_digest: digest('7'), workspace_id: 'workspace-main', project_id: 'project-outcome', role: 'builder', binding_version: 1, expected_schema_version: 1, expected_durable_revision: 0, restored_at: AT }), 'restore_denied')
  } finally { await db.close() }
})
...
test('tombstone purges source and certificate residue instead of retaining deleted raw rows', async () => {
  ...
  try {
    const adapter = createObserverBridgePostgresAdapter({ with_transaction: transactionPort(db) })
    await adapter.tombstone({ workspace_id: 'workspace-main', project_id: 'project-outcome', role: 'builder', binding_version: 1, deletion_revision: 1, deletion_receipt_digest: digest('8'), purge_before: AT, tombstoned_at: EXPIRES, expected_durable_revision: 0 })
