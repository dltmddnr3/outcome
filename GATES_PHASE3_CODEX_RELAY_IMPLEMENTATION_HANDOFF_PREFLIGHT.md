...
- [x] H8: Builder handoff의 allowed files, forbidden mutations, red-first tests, evidence receipt, rollback과 stop conditions가 명시되고 문서·Package 검증이 통과한다.
  ...
  EXPECT: H8_PASS
  EVIDENCE: current handoff SHA-256 `e939f5ad3fa60701730aaada756e46627de2f0a12cef461a81ddd4ee729c003f`; initial Builder slice는 technical spike only/PREPARED_NOT_DISPATCHED이고 exact brief는 AWAITING_PRIVATE_BUILDER_BINDING이다. allowed/forbidden, red-first probes, receipt와 SAFE_HOLD stop conditions를 고정했다. Current regression evidence는 `GATES_PHASE3_FIRST_PROOF_BEHAVIOR_PREFLIGHT.md`에 기록한다. 외부 mutation 없음. current source forced check `H8_PASS`.
