...
인증 실패 상태 교정 `C1-C5 5/5`와 Preview 배포 `D1-D6 6/6`은 완료됐다. Clerk Development의 모바일 검수 세션 단일 철회 뒤 일반 로그인 화면으로 오분류되고 private API `401`이 발생하지 않아 실제 철회 흐름은 `FAIL · 교정 필요`다. 실제 만료·제공자 장애도 아직 관측되지 않았다.

## 현재 관측

| 환경 | 흐름 | 상태 | 근거 또는 다음 동작 |
| --- | --- | --- | --- |
| MacBook Chrome | Google 로그인 | `OPEN` | popup/finalize 교정 후 현재 후보에서 owner 로그인 재확인 필요 |
...
| MacBook Chrome | Google 재로그인 복구 | `OPEN` | popup/finalize 교정 후 현재 후보에서 준비 화면 복귀 재확인 필요 |
...
| 모바일 시스템 브라우저 | Google 로그인 | `PASS` | 현재 후보에서 Cherry 직접 확인 + session/workspace HTTP `200/200` |
...
| 모바일 시스템 브라우저 | email code 대체 로그인 | `PASS` | Cherry 직접 확인 + session/workspace HTTP `200/200`; 이메일·코드 미기록 |
| 모바일 시스템 브라우저 | 세션 만료 | `OPEN` | 만료된 세션의 다음 요청이 로그인 화면으로 fail-closed 되는지 확인 필요 |
| 모바일 시스템 브라우저 | 운영자 세션 철회 | `FAIL · 교정 필요` | 단일 철회 뒤 일반 로그인으로 오분류; 만료 화면·다시 로그인·private API `401` 부재, 새 candidate 재검수 필요 |
| 모바일 시스템 브라우저 | 인증 제공자 장애 | `OPEN` | 통제된 Preview-only 장애 주입 후보와 복구 확인 필요 |

## 다음 안전 실행 순서

1. 외부 설정 변경 없이 현재 후보의 MacBook Google 로그인과 재로그인 복구를 먼저 확인한다.
2. MacBook에서 동일 owner의 email code 대체 로그인을 확인한다. 코드·이메일·계정명은 기록하지 않는다.
