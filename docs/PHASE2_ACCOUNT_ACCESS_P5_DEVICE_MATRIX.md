...
인증 실패 상태 교정 `C1-C5 5/5`와 Preview 배포 `D1-D6 6/6`은 완료됐다. Clerk Development의 모바일 검수 세션 단일 철회는 실행됐지만, 제품 화면·API `401`·재로그인 복구가 아직 관측되지 않아 실제 철회 PASS와 수치는 그대로 열린다. 실제 만료·제공자 장애도 아직 관측되지 않았다.
...
| 모바일 시스템 브라우저 | 운영자 세션 철회 | `관측·복구 대기` | 단일 모바일 검수 세션 철회 실행 완료; 만료 화면·private API `401`·새 로그인 `200/200` 확인 전 PASS 금지 |
