...
- Next: `P5 · 실제 기기 로그인·불러오는 중·준비·로그아웃·만료·철회·제공자 장애`; P4는 허용된 여섯 환경 이름의 Preview-only 연결, 환경 변경 전후 두 exact commit-fixed Preview, 공개 Production 불변을 직접 검증해 닫혔다. current candidate `4613372adbec17e35c2498e55ab4210cc8b33c34`의 P5 실기기 행렬은 직접 관측 `10/19`이며 MacBook Google 로그인·재로그인·email code, 양쪽 만료·인증 제공자 장애와 MacBook 철회가 남아 있다. 모바일 운영자 철회는 Clerk Development의 단일 검수 세션에 실행됐지만 만료 화면·private API `401`·새 로그인 `200/200` 관측과 복구 전이라 PASS 및 수치에 반영하지 않았다. 모바일 Google과 email code 로그인은 exact Preview `dpl_3MYfocjsQ6XvTrCoTNXE6Pp4U7wY`에서 Cherry 직접 확인과 비민감 session/workspace HTTP `200`으로 재검증됐다. Production·Supabase·DNS·도메인·출시는 계속 미승인이다.
- P5 controlled failure preflight: 모바일 철회·제공자 장애·만료를 서로 분리한 `GATES_PHASE2_ACCOUNT_ACCESS_P5_CONTROLLED_FAILURE_PREFLIGHT.md` `F1-F7 7/7` 준비 계약이 있다. 모바일 철회 실행은 `GATES_PHASE2_ACCOUNT_ACCESS_P5_MOBILE_REVOCATION_EXECUTION.md`와 비민감 영수증으로 고정됐고 사용자 관측·재로그인 복구를 기다린다. 제공자 장애와 만료는 각각 실행 직전 10분 유효 단일 사용 Cherry 승인이 필요하며, 준비나 철회 실행만으로 실기기 PASS가 되지 않는다.
