...
- Next: `P5 · 실제 기기 로그인·불러오는 중·준비·로그아웃·만료·철회·제공자 장애`; P4는 허용된 여섯 환경 이름의 Preview-only 연결, 환경 변경 전후 두 exact commit-fixed Preview, 공개 Production 불변을 직접 검증해 닫혔다. current candidate `4613372adbec17e35c2498e55ab4210cc8b33c34`의 P5 실기기 행렬은 직접 관측 `9/19`, 미실행 `10/19`이며 MacBook Google 로그인·재로그인과 MacBook/mobile email code·만료·철회·인증 제공자 장애가 남아 있다. 모바일 Google 로그인은 exact Preview `dpl_3MYfocjsQ6XvTrCoTNXE6Pp4U7wY`에서 Cherry 직접 확인과 config/session/workspace `10/10` HTTP `200`으로 재검증됐다. Production·Supabase·DNS·도메인·출시는 계속 미승인이다.
