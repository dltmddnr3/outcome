...
- Next: `P5 · 실제 기기 로그인·불러오는 중·준비·로그아웃·만료·철회·제공자 장애`; P4는 허용된 여섯 환경 이름의 Preview-only 연결, 환경 변경 전후 두 exact commit-fixed Preview, 공개 Production 불변을 직접 검증해 닫혔다. P5는 MacBook과 모바일 시스템 브라우저에서 실제 흐름을 실측해야 닫히며 Production·Supabase·DNS·도메인·출시는 계속 미승인이다.
