...
- Current: `outcome-phase-2 / outcome-phase-2-account-service / outcome-stage-account-access-hosted-identity-preview · P4 Vercel Preview-only environment`
- Next: `P4 · Preview 전용 환경값과 immutable deployment`; P3는 Google·email code 실제 로그인, 인증 후 Apple 연결 전용, Apple 직접 로그인 비노출, 다른 사용자 거부, 로그아웃·서버 세션 철회를 모두 직접 검증해 닫혔다. P4는 허용된 여섯 환경 이름을 값 비노출로 Preview에만 연결하고 새 commit-fixed Preview를 검증하면서 Production 기준선을 유지해야 닫힌다. Production·Supabase·DNS·도메인·출시는 계속 미승인이다.
