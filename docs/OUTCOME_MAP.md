...
- Next: `P5 모바일 철회 UX correction Preview 승격 승인`; current Preview의 단일 모바일 철회 실기기 검수는 private payload 제거와 SDK no-request 경계는 안전했지만, 이전 ready tab을 일반 첫 로그인으로 오분류해 `FAIL · CORRECTION REQUIRED`다. Builder candidate `5686120d03b821c2aa4f10ea989ff67af07a759a` / tree `a90f43649dc8de0070b0450aff17dfa97a80933e`는 tab-scoped boolean으로 최초 로그인·예상 밖 signed-out·명시적 logout/retry를 구분하고 Parent 회귀를 통과했다. P5 행렬은 `10/19`로 유지하며 push·Preview deploy·별도 승인 철회 재검수는 아직 실행하지 않았다. MacBook Google 로그인·재로그인·email code, 양쪽 만료·제공자 장애와 MacBook 철회도 남아 있다. Production·Supabase·DNS·도메인·출시는 계속 미승인이다.
